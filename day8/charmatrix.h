...
namespace aoc
{
	...
	Vec2 operator*(int scalar, Vec2 vec)
	{
		return Vec2(vec.x * scalar, vec.y * scalar);
	}

	Vec2 operator*(Vec2 vec, int scalar)
	{
		return scalar * vec;
	}
	...
	struct CharMatrix
	{
		...
		/// <param name="c"></param>
		/// <returns></returns>
		bool setCharAt(Vec2 pos, char c)
		{
			if (pos.x < 0 || pos.y < 0)
				return false;
			if (static_cast<uint64_t>(pos.x) >= width || static_cast<uint64_t>(pos.y) >= height)
				return false;
